...
""" 
def convert(num):
    #create an empty string variable to hold out eventual output
    roman = ''
...
################
if num / 1000 > 0:
    #to get the number M's we need
    m = floor(num/1000)
    # Loop m times and add that number of M's to roman 
    for i in range(m):
        roman =+ 'M'
    # now subtract that part accounted for from our number
...
# get the number of 100's
    ...
    c = floor(num / 100)
# get a remainder when dividing by 5
...
if remainderWhenDividingBy5 >= 1 and remainderWhenDividingBy5 <= 3
    ...
    # we might have to ass a D first of the number is in the 600, 700, or 800 range
